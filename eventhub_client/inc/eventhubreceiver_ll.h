...
MU_DEFINE_ENUM(EVENTHUBRECEIVER_RESULT, EVENTHUBRECEIVER_RESULT_VALUES);
...
*  <blockquote>
*    <pre>Endpoint=sb://[Event Hub name goes here].[Event Hub suffix goes here, e.g., servicebus.windows.net];SharedAccessKeyName=[Policy key name here];SharedAccessKey=[Policy key]</pre>
*  </blockquote>
*
...
/**
* @brief    Creates a EventHubReceiver_LL handle for communication with an existing
...
/**
* @brief    Asynchronous call to refresh the SAS token for the specified @p eventHubReceiverLLHandle.
*           This API is to be used only if EventHubReceiver_LL_CreateFromSASToken was used to create
*           the EventHub handle.
*
* @param    eventHubReceiverLLHandle    The handle created by a call to the
*                                       EventHubReceiver_LL_CreateWithSASToken function.
